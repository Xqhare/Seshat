...
impl Editor {
    ...
    pub fn default() -> Self {
        ...
        let mut initial_status = String::from("HELP: Ctrl-s to save | Ctrl-f to find | Ctrl-q to quit");
        // (Xqhare): Dont forget args[0] is the app name!
    ...
    pub fn refresh_screen(&self) -> Result<(), std::io::Error> {
        ...
        if self.should_quit {
            ...
            println!("All done! Have a great day!\r");
        } else {
            self.draw_rows();
    ...
    fn search (&mut self) {
        ...
        if let Some(query) = self.prompt("Search (ESC t ocancel, Arrows to navigate): ", |editor, key, query| {
            let mut moved = false;
            match key {
                Key::Right | Key::Down => {
    ...
    pub fn process_keypress(&mut self) -> Result<(), std::io::Error> {
        ...
        match pressed_key {
            ...
            Key::Backspace => {
                // (Xqhare): Backspace consists of two operations, move left and delete!
                // (Xqhare): the || is an xor I believe
    ...
    fn prompt<C>(&mut self, prompt: &str, callback: C) -> Result<Option<String>, io::Error> where C: Fn(&mut Self, Key, &String), {
